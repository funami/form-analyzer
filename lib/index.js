...
(() => {
  ...
  var waitAppearTarget = (window2, targetlement, appearThreshold = 0.4) => {
    const fire = () => {
      return window2.scrollY + window2.innerHeight * (1 - appearThreshold) > targetlement.offsetTop;
    };
    return new Promise((resolve, reject) => {
      if (targetlement == null) {
        ...
        if (fire()) {
          ...
          const scrollWatcher = () => {
            if (fire()) {
              window2.removeEventListener("scroll", scrollWatcher);
              resolve();
            }
          };
  ...
  var FormMeasure = class {
    ...
    start() {
      ...
      formList.forEach((_form, i) => {
        ...
        waitAppearTarget(this.window, form).then(() => {
    ...
    getFormId(form, index = 0) {
      return form.getAttribute("id") || form.getAttribute("name") || form.getAttribute("class") || `FORM_INDEX[${index}]`;
    }
